...
| 🤖 | **内置 AI（非外挂）** | AI 中台统一接入多模型（GLM/Qwen/私有化 vLLM）：文档智能解析、混合语义搜索、对话式 AI 助手 |
...
- **mono 单进程模式**：`PROFILE=mono` 两进程跑全栈（10 模块聚合单上下文 + 独立 gateway），实测 RSS **405MB（较 11 服务独立部署 -78%）**。
...
| [mono 单进程设计](docs/OpenForge-mono单进程设计.md) | 10 模块聚合、-78% 内存实测 |

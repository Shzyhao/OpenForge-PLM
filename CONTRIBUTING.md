...
感谢你关注 OpenForge PLM！本文档说明如何参与贡献。项目 v1.1.0 应用主体与框架化 F1~F3 已交付，最需要的是后续路线共建与场景输入。
...
1. **设计评审**：对架构与设计文档提出 Issue（架构、数据模型、AI 安全机制、流程引擎），标注 `design-review`；
2. **模块认领**：认领后续路线模块（事件总线 / 可观测 / K8s Helm 部署 / ERP/MES 连接器，见 Roadmap），在对应 Issue 下留言认领；
3. **代码贡献**：遵循下方规范；
...
- 后端：Spring Boot 3.x；前端：React 18 + Vite；AI：FastAPI（OpenAI 兼容协议接入）
- 本地依赖（PostgreSQL/Redis/MinIO 等）统一用 `docker-compose` 启动

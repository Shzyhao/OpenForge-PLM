...
> 状态：**刀 1（骨架）已实施并通过全栈实测（PR #98）**；刀 2（进程内直调）评估完成不实施，见 §3.2
> 后续并入：connector（v1.14.0）、drawing（v1.20.0）——mono 现聚合 **10 模块**（本文 mono-8 为设计时基线）
> 调研基线：v1.11.0（dev 8ef2449），跨服务调用 13 处 RestClient 调用点全量核对
> 实测快照（PROFILE=mono，2026-09-05）：**mono 224MB + gateway 181MB = 405MB RSS**
> （独立部署 9 进程 1872MB → **-78%**，超设计目标 ≤1GB）；Spring 启动 mono 3.9s / gateway 2.4s；
> 网关链路冒烟 **8/8 域全绿**（brokenModules/routeMissing 空，ai-gateway 未启动心跳摘除属正常降级）

## 4.1 实测数据对照（刀 1 交付）

| 维度 | full（9 进程实测） | mono（2 进程实测） | 变化 |
|------|-------------------|-------------------|------|
| JVM RSS 合计 | 1872MB | 405MB（224+181） | **-78%** |
| Hikari 连接池 | 9×2=18（dev 环境参数） | 1×2=2 | -89% |
| 单服务 Spring 启动 | 2.0~2.8s | mono 3.9s | 单上下文承载 8 服务，仍 ~4s |
| 网关链路冒烟 | 8/8 域 | 8/8 域 | 等价 |
| CDS | 各服务 ~16MB 档案 | mono 档案 15MB（自动训练） | 同机制复用 |
| 行为面 | — | module-routes/DEGRADED/心跳摘除/信任头模型原样 | 等价 |
...
**刀 2 评估结论（刀 1 实测后判定，2026-09-05）：不实施。**
5 组回环调用全部有 TTL 缓存（权限 60s / 模块可用性 30s）或为低频业务动作（取号/流程启动/Schema 沉淀——伴随创建与发布操作，单次本机回环 <1ms）；刀 1 实测（RSS 405MB / 启动 3.9s / 冒烟 8/8 域）无任何回环相关瓶颈。而直调化必须二选一：侵入 4 个服务模块的客户端类（破坏刀 1"独立部署零变化"边界），或 mono 下子类覆写（脆，鉴权/取号/发布语义回归风险）。收益≈零、风险不成比例——按画像 §5 自检判停。若未来出现跨进程观测需求（trace 断链/回环超时事故）再重启评估。

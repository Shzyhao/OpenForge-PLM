...
1. **P2-2 已全量交付（v1.15.0）**，集成编排器 MVP + P2 两期闭环。后续候选：多步骤画布编排（P3，NodeConfigPanel 已预留）、密钥轮换批处理（R1）、出站脱敏代理与 SSRF TOCTOU 根治合并实施（R6）、material 域事件化后 part.released 主题入白名单（B2 P3）、按连接器 ACL（Q2）

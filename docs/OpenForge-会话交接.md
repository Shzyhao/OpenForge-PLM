...
2. **单进程 mono 模式（9 合 1 JVM，-2GB 大项）**：**设计刀已完成（docs/OpenForge-mono单进程设计.md，v1.11.0 后会话交付）**——mono-8 + 独立 gateway 两刀方案（gateway 因 WebFlux/WebMVC 自动配置互斥不并入；资源目录化解 8 组同名根资源遮蔽；刀 1 零客户端改造、刀 2 进程内直调），**待评审后排期实施**；H2 文件库 dev 模式维持 §8.3 备选不动
3. **连接器与行业模板包**：需外部场景输入

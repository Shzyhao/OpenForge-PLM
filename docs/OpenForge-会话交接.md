...
1. **v1.16.0 已发版**（PR #111 → main + tag + Release + 回灌）。集成编排器 P2 + P3 前两项（material 事件化/密钥轮换）闭环，无在途功能。剩余候选：多步骤画布编排（P3 旗舰，建议先 spike）、SSRF 根治+出站代理（R6，生产前）、连接器级 ACL（Q2）、新内置连接器类型、多模型分流（AI 中台）、规模化基建（Redis/Milvus/ES 等随规模信号）
